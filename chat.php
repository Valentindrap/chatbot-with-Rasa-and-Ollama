...
<body data-theme="light">
  ...
  <div class="chat-wrapper">
    ...
    <!-- CONTROLES VOZ -->
    <div id="speech-controls" style="padding:0 1rem 0 1rem;">
      <div style="display:flex; gap:8px; align-items:center; margin:10px 0;">
        <button id="recordBtn" class="glow-effect" style="padding:0.6rem 0.9rem; border-radius:10px;">
          <i class="fas fa-microphone"></i> Grabar
        </button>
        <button id="stopBtn" class="glow-effect stop-btn" disabled style="padding:0.6rem 0.9rem; border-radius:10px;">
          <i class="fas fa-stop"></i> Parar
        </button>
        <span id="speechStatus" style="margin-left:10px; color:var(--text-secondary)">Estado: inactivo</span>
        <div style="margin-left:auto; display:flex; gap:8px; align-items:center;">
          <label for="speechLang" style="font-size:0.9rem; color:var(--text-secondary);">Idioma:</label>
          <select id="speechLang" style="padding:6px; border-radius:8px;">
      ...
      <small style="color:var(--text-secondary);display:block;margin-bottom:8px;">La transcripción se colocará directamente en el campo de entrada y se <strong>enviará automáticamente</strong> cuando finalice la grabación.</small>
    </div>
  ...
  // ---------------------- WEB SPEECH API (solo escribe en el input, no envía) ----------------------
(function(){
  const recordBtn = document.getElementById('recordBtn');
  const stopBtn = document.getElementById('stopBtn');
  const statusSpan = document.getElementById('speechStatus');
  const langSelect = document.getElementById('speechLang');
  const input = document.getElementById('user-input');

  // Comprueba soportepeechRecognition 
  const SpeechRecognition = window.S|| window.webkitSpeechRecognition;
  if (!SpeechRecognition) {
    statusSpan.innerText = 'Estado: tu navegador no soporta Web Speech API';
  ...
  function createRecognition() {
    ...
    rec.lang = langSelect?.value || 'es-AR';
    rec.interimResults = true;
    rec.continuous = false; // grabación por frase
    rec.maxAlternatives = 1;

    rec.onstart = () => {
      finalTranscript = '';
      statusSpan.innerText = 'Estado: grabando...';
      recordBtn.disabled = true;
    ...
    rec.onresult = (event) => {
      ...
      // Escribir interim + final directo en el input (sin enviar)
    ...
    rec.onerror = (ev) => {
      ...
      if (ev.error === 'not-allowed' || ev.error === 'service-not-allowed') {
        statusSpan.innerText = 'Permiso micrófono denegado.';
      } else if (ev.error === 'network') {
        statusSpan.innerText = 'Error de red del servicio de reconocimiento.';
      } else {
        statusSpan.innerText = 'Error de reconocimiento: ' + (ev.error || ev.message || 'unknown');
      }
    ...
    rec.onend = () => {
      // Cuando termina, dejamos el texto en el input y habilitamos botones.
      statusSpan.innerText = 'Estado: terminado';
      recordBtn.disabled = false;
      stopBtn.disabled = true;
      // NO auto-send: dejamos el texto para que el usuario lo revise y pulse enviar.
    };
  ...
  recordBtn.addEventListener('click', () => {
    ...
    try {
      ...
      // start() puede lanzar si ya está corriendo; reiniciamos por seguridad
      try { recognition.stop(); } catch(_) {}
      recognition = createRecognition();
      try { recognition.start(); } catch(err) { console.warn('No se pudo iniciar reconocimiento', err); }
    }
  });

  // Botón parar (detiene la captura y mantiene el texto en el input)
 ...
 // ---------------------- FIN WEB SPEECH API (solo escribe en el input) ----------------------
